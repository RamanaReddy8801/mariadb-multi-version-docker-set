...
console.log("New Relic agent status:", newrelic.agent?.config?.agent_enabled ?? "unavailable");
...
async function startTestApp() {
  ...
  app.use(requestLogger("mysql-app"));

  app.get("/health", (req, res) => {
    res.json({ status: "ok" });
  });

  // 1. Search by hire_date — 4-table JOIN, full scan on hire_date (no index)
  ...
  // 2. Admin employee search — complex query with derived tables + leading LIKE wildcards
  ...
  // 3. Bulk title update with transaction
  app.put("/admin/employees/bulk_title_update", async (req, res) => {
    ...
    try {
      ...
      // End current titles for Engineers in dept d005
  ...
  // 4. Department management audit — correlated subquery per dept
  ...
  // 5. Employee details with full history — GROUP_CONCAT correlated subqueries per row
    ...
    try {
      ...
      const [rows] = await connection.query(`
        SELECT
            ...
            (SELECT GROUP_CONCAT(
               CONCAT(dept_name, ': ', de2.from_date, ' to ',
  ...
  // 6. Salary audit — correlated subqueries per row for previous salary and dept avg
  ...
  // 7. Transfer audit — self-join on dept_emp with NOT EXISTS anti-join
  ...
  // 8. Data export — intentional connection leak pattern (10 connections, delayed release)
  app.get("/admin/employees/data_export", async (req, res) => {
    ...
    try {
      ...
      // Delayed release to simulate connection leak behaviour
      setTimeout(() => {
  ...
  app.listen(port, () => console.log(`mysql-app running on port ${port}`));
}
